...
## 2026-09-18 22:48:44 UTC
- NEW vps.signl4.com dangling A-record confirmed: GoDaddy authoritative NS (domaincontrol.com) → 72.167.227.27 (secureserver.net shared host); HTTPS 200 nginx "Default Web Site Page" with cert CN=server.d1g
- NEW V4 API namespace confirmed absent: connect.signl4.com/api/v4 → 404, /api/docs/v4/swagger.json → 404 — sole unprobed namespace gap closed
- CHANGED api.signl4.com/api/v2/teams auth-status flapping re-confirmed 10th+ cycles: unauth GET → 401 (was 405 prior cycle), invalid Bearer → 401 — handler/routing auth layer unstable across cycles
...
- CHANGED connect.signl4.com/api/v1/alerts/acknowledgeAll: unauth GET → 401, invalid Bearer → 401 — route-gated this cycle (differs from V2 teams flapping)
- CHANGED www.vps.signl4.com: NXDOMAIN (CT cert token only, no DNS record)
- CHANGED go.signl4.com: Cloudflare 403 error 1034 — third-party/CDN config, not Derdack defect
...
## 2026-09-19 19:09:47 UTC
- NEW docs.enterprisealert.com: A-record → Azure edge serving HTTP 404 with default *.web.core.windows.net cert + no hostname SAN — decommissioned/dangling Azure static hosting binding (confidence 45, claim
...
- CHANGED vps.signl4.com subdomain takeover re-confirmed: HTTPS 200 nginx default page, cert CN=server.d1g.it, reverse DNS secureserver.net, GoDaddy authoritative NS
- CHANGED Cross-env token forgery chain stable at 6 identity hosts (connect, devconnect, api, devapi, account, devaccount) — byte-identical RS256 JWKS, shared client_id 692A0A56, password grant enabled on stagi
- CHANGED api.signl4.com/api/v2/teams auth-status flapping 10th+ cycles (unauth GET 401 vs 405, invalid Bearer 401) — handler/routing layer unstable
- CHANGED connect.signl4.com/api/v3/subscriptions/{sub}/invoices/{inv}/zugferd: unauth GET 401, invalid Bearer 401 — route-gated (differs from V1/V2 handler-deferred)
- CHANGED connect.signl4.com/api/v1/alerts/acknowledgeAll: unauth GET 401, invalid Bearer 401 — route-gated this cycle (differs from V2 teams flapping)
...
## 2026-09-20 22:49:26 UTC
- NEW vps.signl4.com subdomain takeover CONFIRMED via live probes (2026-09-18 through 2026-09-20): HTTPS 200 nginx default page (`/cgi-sys/defaultwebpage.cgi`), TLS cert CN=server.d1g.it (unrelated), GoDadd
- NEW docs.enterprisealert.com CNAME chain corrected: `→ derdackdownloads.z13.web.core.windows.net` (live Azure SWA, 404 WebContentNotFound) — NOT dangling A-record; takeover confidence ≤25
- NEW us.derdack.com live: MultiViews `300 Multiple Choices` byte-identical to dev.derdack.com (/.well-known/→300 listing /.ssh//.bash_history//.viminfo/, /.ssh/→403, fabricated paths→300)
- NEW s4dev1-8.enterprisealert.com CNAME → Azure AD App Proxy (`*.msappproxy.net`) → 4.207.244.99 — staging fleet App-Proxy-fronted, explains 404/504 inertness
- NEW CT coverage complete: %derdack.com (4) + %enterprisealert.com (5) sweeps done — zero new live names, estate drift = 0
...
## 2026-09-22 05:01:35 UTC
- NEW vps.signl4.com subdomain takeover CONFIRMED via live probes (2026-09-18 through 2026-09-22): HTTPS 200 nginx default page (`/cgi-sys/defaultwebpage.cgi`), TLS cert CN=server.d1g.it (unrelated), GoDadd
- NEW GitHub org sweep complete 12/12 — apiKey-in-URL credential-template family confirmed in 6 repos (nagios, 2wayREST×3, SIGNL4, User-Monitoring) + checkmk hardcoded cred; deployments log keys to EA log/p
- CHANGED docs.enterprisealert.com CNAME chain corrected: `→ derdackdownloads.z13.web.core.windows.net` (live Azure SWA, 404 WebContentNotFound) — NOT dangling A-record; takeover confidence ≤25
- CHANGED us.derdack.com live: MultiViews `300 Multiple Choices` byte-identical to dev.derdack.com (/.well-known/→300 listing /.ssh//.bash_history//.viminfo/, /.ssh/→403, fabricated paths→300)
- CHANGED api.signl4.com/api/v2/teams auth-status flapping 10th+ cycles (unauth GET 401 vs 405, invalid Bearer 401) — handler/routing layer unstable
...
## 2026-09-23 02:34:14 UTC
- NEW demo.enterprisealert.com/EAWebService/: Full live EA 9.5.26147 web portal discovered behind Azure AD App Proxy (previously only root "/" probed → 404; path-family probing finds live sub-app with Event
- NEW us.derdack.com: Live MultiViews 300 with byte-identical dot-prefix echo to dev.derdack.com (/.well-known/, /.ssh/, /.bash_history/, /.viminfo/) — second host confirming estate-wide parked-minimal conf
- NEW GitHub org sweep complete 12/12: apiKey-in-URL credential-template family confirmed in 6 repos (nagios, 2wayREST×3, SIGNL4, User-Monitoring) + checkmk hardcoded cred; deployments log keys to EA log/pr
- NEW SolarWinds integration article (derdack.com/de/): Cleartext credential URL template `EventProviderAPI.aspx?Handler=RaiseEvent&Username=services&Password=Derdack01&...` + internal hostname `mmea2012.de
- NEW EA9 HTTP/SOAP API PDF: Documents EventProviderAPI `Password` param accepts "String or **md5 hashed**" — MD5-hash-in-URL credential variant vendor-documented
...
## 2026-09-23 08:00:46 UTC
- NEW demo.enterprisealert.com/EAWebService/ — Full live EA 9.5.26147 portal discovered behind Azure AD App Proxy (previously only root "/" probed → 404; path-family probing finds live sub-app with EventPro
- NEW us.derdack.com — Live MultiViews 300 with byte-identical dot-prefix echo to dev.derdack.com (/.well-known/, /.ssh/, /.bash_history/, /.viminfo/) — second host confirming estate-wide parked-minimal con
- NEW GitHub org sweep 12/12 complete — apiKey-in-URL credential-template family confirmed in 6 repos (nagios, 2wayREST×3, SIGNL4, User-Monitoring) + checkmk hardcoded cred; deployments log keys to EA log/p
- NEW SolarWinds integration article (derdack.com/de/) — Cleartext credential URL template `EventProviderAPI.aspx?Handler=RaiseEvent&Username=services&Password=Derdack01&...` + internal hostname `mmea2012.d
- NEW EA9 HTTP/SOAP API PDF — Documents EventProviderAPI `Password` param accepts "String or **md5 hashed**" — MD5-hash-in-URL credential variant vendor-documented
- CHANGED vps.signl4.com subdomain takeover — Re-confirmed across 5 consecutive probe cycles (2026-09-18 through 2026-09-22) — HTTPS 200 nginx default page, TLS cert CN=server.d1g.it, GoDaddy authoritative NS, 
- CHANGED docs.enterprisealert.com — CNAME chain corrected to live Azure SWA `derdackdownloads.z13.web.core.windows.net` (404 WebContentNotFound) — NOT dangling A-record; takeover confidence ≤25
- CHANGED Cross-env token forgery chain — Stable at 6 identity hosts (connect, devconnect, api, devapi, account, devaccount) — byte-identical RS256 JWKS, shared client_id 692A0A56, password grant enabled on sta
- CHANGED api.signl4.com/api/v2/teams auth-status flapping — 10th+ cycles (unauth GET 401 vs 405, invalid Bearer 401) — handler/routing layer unstable
- CHANGED Leak-oracle for example EA key `l1slfpjwirbm6twzs30bhtcl3z34m8v9` — Hits ONLY on vendor Zendesk/portfolio — zero customer-deployed instance across 7+ corpora

## 2026-09-23 13:18:39 UTC
- NEW demo.enterprisealert.com/EAWebService/: Full live EA 9.5.26147 portal discovered behind Azure AD App Proxy (previously only root "/" probed → 404; path-family probing finds live sub-app with EventProv
- NEW us.derdack.com — Live MultiViews 300 with byte-identical dot-prefix echo to dev.derdack.com (/.well-known/, /.ssh/, /.bash_history/, /.viminfo/) — second host confirming estate-wide parked-minimal con
- NEW GitHub org sweep 12/12 complete — apiKey-in-URL credential-template family confirmed in 6 repos (nagios, 2wayREST×3, SIGNL4, User-Monitoring) + checkmk hardcoded cred; deployments log keys to EA log/p
- NEW SolarWinds integration article (derdack.com/de/) — Cleartext credential URL template `EventProviderAPI.aspx?Handler=RaiseEvent&Username=services&Password=Derdack01&...` + internal hostname `mmea2012.d
- NEW EA9 HTTP/SOAP API PDF — Documents EventProviderAPI `Password` param accepts "String or **md5 hashed**" — MD5-hash-in-URL credential variant vendor-documented
...
## 2026-09-23 17:57:04 UTC
- NEW demo.enterprisealert.com/EAWebService: Full live EA 9.5.26147 portal discovered behind Azure AD App Proxy (previously only root "/" probed → 404; path-family probing finds live sub-app with EventProvi
- NEW us.derdack.com: Live MultiViews 300 with byte-identical dot-prefix echo to dev.derdack.com (/.well-known/, /.ssh/, /.bash_history/, /.viminfo/) — second host confirming estate-wide parked-minimal conf
- NEW vps.signl4.com subdomain takeover: Re-confirmed across 5+ consecutive probe cycles — HTTPS 200 nginx default page, TLS cert CN=server.d1g.it, GoDaddy authoritative NS, fabricated-host byte-identical r
- NEW Cross-env token forgery chain: Stable at 6 identity hosts (connect, devconnect, api, devapi, account, devaccount) — byte-identical RS256 JWKS (kid 91EE4F3CE94EB517AF66B254F7497ECB0E31EE27RS256), share
- NEW api.signl4.com/api/v2/teams auth-status flapping: 10th+ cycles (unauth GET 401 vs 405, invalid Bearer 401) — handler/routing layer unstable
- NEW Credential-in-URL vendor-template family: 7+ distinct sources (SolarWinds article, EA9 PDF, send-alerts-web-services, nagios, 2wayREST×3, SIGNL4, User-Monitoring) with md5-hash variant documented
- NEW Leak-oracle for example EA key `l1slfpjwirbm6twzs30bhtcl3z34m8v9`: Hits ONLY on vendor Zendesk/portfolio — zero customer-deployed instance across 7+ corpora
- NEW RAG on `mmea2012.derdack-support.local` + `Password=Derdack01`: Zero indexed hits outside SolarWinds article — example-only, no reuse
- CHANGED Full estate identity/API/CT surface stable zero drift; passive route maps exhausted
- CHANGED V4 API namespace confirmed absent: connect.signl4.com/api/v4 404, /api/docs/v4/swagger.json 404 — sole unprobed namespace gap closed
- CHANGED signl4.derdack.com permanently unreachable (8+ cycles TCP timeout) — attack surface value = 0
...
## 2026-09-23 23:26:49 UTC
- CHANGED demo.enterprisealert.com/EAWebService: handler/op enumeration now complete (11 .aspx handlers, 12-op .asmx WSDL, EventConnectorServer.ashx 200-empty) — all anon-readable, all credential-paths gate (Er
- CHANGED Credential-in-URL family: 7+ vendor sources (SolarWinds `Password=Derdack01`+`mmea2012.derdack-support.local`, 2016 send-alerts, EA9 PDF md5-hash variant, 6/12 repos) with leak-oracles negative across
...
- NEW demo.enterprisealert.com/EAWebService: Full live EA 9.5.26147 portal confirmed behind Azure AD App Proxy (root "/" was 404; /EAWebService/ path-family probing found live sub-app). EventProviderAPI.asp
- NEW vps.signl4.com: Subdomain takeover CONFIRMED via live probe — HTTPS 200 nginx default page (`/cgi-sys/defaultwebpage.cgi`), TLS cert CN=server.d1g.it (unrelated Let's Encrypt), GoDaddy authoritative N
- NEW us.derdack.com: Live MultiViews 300 with byte-identical dot-prefix echo to dev.derdack.com (/.well-known/ → 300 listing /.ssh/, /.bash_history/, /.viminfo/, /.ssh/ → 403) — second host confirming esta
- CHANGED Cross-env token forgery chain: Stable at 6 identity hosts (connect, devconnect, api, devapi, account, devaccount) — byte-identical RS256 JWKS (kid 91EE4F3CE94EB517AF66B254F7497ECB0E31EE27RS256), share
- CHANGED Credential-in-URL vendor-template family: 7+ distinct sources (SolarWinds article, EA9 PDF, send-alerts-web-services, nagios, 2wayREST×3, SIGNL4, User-Monitoring) with md5-hash variant documented; lea
- CHANGED api.signl4.com/api/v2/teams auth-status flapping: 10th+ cycles (unauth GET 401 vs 405, invalid Bearer 401) — handler/routing layer unstable.
- CHANGED Full estate identity/API/CT surface stable zero drift; passive route maps exhausted.

## 2026-09-24 01:40:06 UTC
- CHANGED nemotron3 ranked hypothesis "[85] Unauthenticated EventProviderAPI mutating handler access" re-checked against my PASSIVE-verified gate data (2026-09-22/23): all 11 handlers and 12 WSDL ops reject fab
...
## 2026-09-26 10:00:42 UTC
- NEW `demo.enterprisealert.com/EAWebService/` directory index (200, HTML titled "Enterprise Alert® Event Provider API & Connector SDK (v9.5.26147)") — never probed before, links 3 endpoint docs
- NEW `demo.enterprisealert.com/EAWebService/EventConnectorServer.ashx` renders empty installed-handlers table (200, 240B, `?action=GetStatus` byte-identical) — closes HIGH-value Connector SDK alert-exfil c
- NEW `s4dev5/6/7.enterprisealert.com/EAWebService/EventProviderAPI.aspx?Handler=About` → bare 404, zero `x-ms-proxy-*` headers (matches s4dev1/2/3/8) — App Proxy identifier disclosure bounded at 2/8 hosts
- NEW GraphQL class closed: `/graphql`, `/api/graphql`, `/api/v2/graphql`, `/api/v3/graphql`, `/graphiql` → 15/15 uniform bare 404 across connect/api/demo
...
- NEW Fresh OIDC discovery on `api.signl4.com` + `account.signl4.com` records `dpop_signing_alg_values_supported: ["RS256"]` — mitigating control
...
## 2026-09-26 14:00:49 UTC
- NEW demo.enterprisealert.com/EAWebService/ directory index (200, HTML titled "Enterprise Alert® Event Provider API & Connector SDK (v9.5.26147)") — never probed before, links 3 endpoint docs
- NEW demo.enterprisealert.com/EAWebService/EventConnectorServer.ashx renders empty installed-handlers table (200, 240B, ?action=GetStatus byte-identical) — closes HIGH-value Connector SDK alert-exfil class
- NEW s4dev5/6/7.enterprisealert.com/EAWebService/EventProviderAPI.aspx?Handler=About → bare 404, zero x-ms-proxy-* headers (matches s4dev1/2/3/8) — App Proxy identifier disclosure bounded at 2/8 hosts
- NEW GraphQL class closed: /graphql, /api/graphql, /api/v2/graphql, /api/v3/graphql, /graphiql → 15/15 uniform bare 404 across connect/api/demo
- NEW Fresh WSDL GET exposes internal single-label SOAP address https://ereesus/EAWebService/EventProviderAPI.asmx — internal hostname leak
- NEW Fresh OIDC discovery on api.signl4.com + account.signl4.com records dpop_signing_alg_values_supported: ["RS256"] — mitigating control
...
- NEW vps.signl4.com subdomain takeover live-verified: HTTPS 200 nginx default page, TLS cert CN=server.d1g.it (unrelated Let's Encrypt), GoDaddy authoritative NS (domaincontrol.com), fabricated-host byte-i
- NEW demo.enterprisealert.com/EAWebService/ directory index (200, HTML titled "Enterprise Alert® Event Provider API & Connector SDK (v9.5.26147)") never probed before, links 3 endpoint docs
- NEW demo.enterprisealert.com/EAWebService/EventConnectorServer.ashx renders empty installed-handlers table (200, 240B, ?action=GetStatus byte-identical) — closes HIGH-value Connector SDK alert-exfil class
- NEW Fresh WSDL GET exposes internal single-label SOAP address https://ereesus/EAWebService/EventProviderAPI.asmx — internal hostname leak
